...
def pole_map(all_func_poles, save_directory="Pole Map", save_fig=False, zoom=True):
    ...
    for i in range(num_polos):
        ...
        if i == 5:
            ax.text(
                real_pt[0][i] - 3.5 * deslocx * width_x,
                imag_pt[0][i] + deslocy * width_y,
                "z1",
                fontsize=fntsz,
                color="red",
...
def pole_map_zoom(real, imag, ftsz, save_directory="Pole Map", save_fig=False):
    ...
    for i in range(num_polos):
        if i != 5:
            ...
            ax1.text(
                real_pt[0][i] + 22 * deslocx * width_x,
                imag_pt[0][i] + deslocy * width_y,
                "z1",
                fontsize=ftsz,
                color="red",
...
def Scatter(
    ...
    for i, (xi, label) in enumerate(zip(x_coords[:8], xlabels)):
        ...
        axs[row, col].set_ylabel(
            "Variação do Polo", fontsize=fntsz_axis, fontweight="bold"
        )
    ...
    for enum, y_coord in enumerate(all_y_coord):
        try:
            Scatter(
                x_coords,
                y_coord,
                pole_name=pole_name[enum],
                fnt_legenda=fnt_legenda,
                fntsz_title=fntsz_title,
                fntsz_axis=fntsz_axis,
                fnt_ticks=fnt_ticks,
                size=size,
                save_fig=save_fig,
            )
...
def histogram(
    ...
    for enum, y_coord in enumerate(data):
        ...
        n, _, patches = ax.hist(
            y_coord, bins=20, color="darkblue", edgecolor="black", alpha=0.7
        )
        ...
        ax.axvline(
            ...
            linestyle="dashed",
            linewidth=1.5,
            label=f"Desvio Padrão {deviation:.2e}",
